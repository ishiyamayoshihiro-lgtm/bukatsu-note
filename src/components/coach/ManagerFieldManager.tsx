...
type Tab = 'manager_practice' | 'manager_expedition'
...
const TABS: { key: Tab; label: string }[] = [
  { key: 'manager_practice',   label: 'マネージャ（練習）' },
  { key: 'manager_expedition', label: 'マネージャ（遠征・大会）' },
]
...
export function ManagerFieldManager({ fields, activeTab: controlledTab }: Props) {
  const [internalTab, setInternalTab] = useState<Tab>('manager_practice')
  const [isPending, startTransition] = useTransition()
  const [editingId, setEditingId] = useState<string | null>(null)

  const activeTab = (controlledTab as Tab | undefined) ?? internalTab
